...
) : Controller
{
    ...
    {
        var item = await dbContext.Products.Include(p => p.Catalogs).Include(p => p.Specs).Include(p => p.ProductImages).SingleOrDefaultAsync(p => p.Id == model.Id);
        ...
        var form = Request.Form;
         dbContext.ProductSpecifications.RemoveRange(item.Specs);
        ...
        .ForEach(p =>
        {
            item.Specs.Add(new ProductSpecification
            {
                SpecificationId = p.Id,
                Value = form[p.Id.ToString()]
            });
